...
schemas_code = """from __future__ import annotations
import enum
...
Path("bitnet_runtime/api/v1/schemas.py").write_text(schemas_code, encoding="utf-8")
print("Wrote bitnet_runtime/api/v1/schemas.py")

import os
from pathlib import Path

gateway_code = """from __future__ import annotations
import json
import logging
import re
import time
...
from ...model_garden import model_garden, model_lifecycle
...
def _build_metadata(
    ...
    if not endpoint and trace and hasattr(trace, "attempts") and trace.attempts:
        ...
        endpoint = last_attempt.get("provider", "local in-process GGUF")
    if not endpoint:
...
async def run_embeddings(req: EmbeddingRequest, x_request_id: Optional[str] = Header(None)):
    ...
    manifest = model_garden.get(req.model_id or "bge_small_en_v1.5") or model_garden.get_default_embedding_model()
...
async def run_rerank(req: RerankRequest, x_request_id: Optional[str] = Header(None)):
    ...
    manifest = model_garden.get("bge_reranker_base") or model_garden.get_default_reranker_model()
    try:
...
Path("bitnet_runtime/api/v1/gateway.py").write_text(gateway_code, encoding="utf-8")
print("Successfully generated bitnet_runtime/api/v1/gateway.py")
